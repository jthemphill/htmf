...
impl Game {
    ...
    pub fn finished_drafting(&self) -> bool {
        self.bot.root_game.state.finished_drafting()
    }

    pub fn game_over(&self) -> bool {
        self.bot.root_game.state.game_over()
    }

    pub fn active_player(&self) -> Option<u8> {
        self.bot.root_game.state.active_player().map(|p| p.id as u8)
    }

    pub fn score(&self, player: usize) -> usize {
        self.bot.root_game.state.get_scores()[player]
    }

    pub fn turn(&self) -> usize {
        self.bot.root_game.state.turn
    }

    pub fn num_fish(&self, idx: u8) -> usize {
        self.bot.root_game.state.board.num_fish(idx)
    }

    pub fn penguins(&self, player: usize) -> Vec<u8> {
        self.bot.root_game.state.board.penguins[player]
            .into_iter()
            .collect()
    }

    pub fn claimed(&self, player: usize) -> Vec<u8> {
        self.bot.root_game.state.board.claimed[player]
            .into_iter()
            .collect()
    }

    pub fn draftable_cells(&self) -> Vec<u8> {
        self.bot.root_game.state.board.fish[0]
            .exclude(self.bot.root_game.state.board.all_claimed_cells())
            .into_iter()
            .collect()
    }
    ...
    pub fn place_penguin(&mut self, dst: u8) -> Result<(), JsValue> {
        let mut new_game = self.bot.root_game.state.clone();
    ...
    pub fn playout(&mut self) {
        if self.bot.root_game.state.game_over() {
            return;
        }
        self.bot.playout();
    ...
    pub fn playout_n_times(&mut self, n: usize) {
        if self.bot.root_game.state.game_over() {
            return;
        }
        for _ in 0..n {
    ...
    pub fn take_action(&mut self) -> Result<(), JsValue> {
        ...
        while self.bot.root_game.state.active_player() == Some(self.bot.me) {
