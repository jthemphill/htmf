...
export default function Board(props) {
    ...
    for (let r = 0; r < NUM_ROWS; ++r) {
        ...
        for (let c = 0; c < rowLength; ++c) {
            ...
            hexes.push(React.createElement(Hex, { key: key, _key: key, onClick: props.handleCellClick, fish: fish, cx: x, cy: y, sideLength: sideLength, highlighted: isHighlighted, possible: possible, player: players.get(key), claimed: claimed.has(key), isTopMoveSrc: props.topMove?.src === key, isTopMoveDst: props.topMove?.dst === key }));
        }
    }
